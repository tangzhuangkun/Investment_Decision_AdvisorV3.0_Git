...
CREATE TABLE IF NOT EXISTS `index_estimation_from_lxr_di`(
	...
	`dyr_mcw` DECIMAL(22,17) NOT NULL COMMENT '股息率市值加权，所有样品公司市值之和 / 分红之和',
	`dyr_ew` DECIMAL(22,17) NOT NULL COMMENT '股息率等权，算出所有公司的DYR，然后通过(n / ∑(1 / DYR.i))计算出来',
	`dyr_ewpvo` DECIMAL(22,17) NOT NULL COMMENT '股息率正数等权，剔除所有不分红的企业',
	`dyr_avg` DECIMAL(22,17) NOT NULL COMMENT '股息率平均值',
	`dyr_median` DECIMAL(22,17) NOT NULL COMMENT '股息率中位数，算出所有样品公司的股息率，然后排序，然后取最中间的那个数；如果是偶数，那么取中间的两个，加和求半',
	`trading_date` DATE NOT NULL COMMENT '交易日期',
	`source` VARCHAR(10) NOT NULL COMMENT '数据来源',
	`submission_date` DATE DEFAULT NULL COMMENT '提交的日期',
	`submission_time` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '提交时间',
