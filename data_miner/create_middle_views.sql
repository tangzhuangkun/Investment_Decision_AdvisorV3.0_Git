...
(select a.index_code, index_name, global_stock_code, stock_code, stock_name, weight, source, a.submission_date
from
/* 每个指数，构成成份，采集日期 */
(select index_code, index_name, global_stock_code,stock_code, stock_name, weight, source, submission_date
from index_constituent_stocks_weight
where source = '聚宽'
group by index_code, index_name, global_stock_code, stock_code, stock_name, weight, source, submission_date
...
group by a.index_code, index_name, global_stock_code, stock_code, stock_name, weight, source, submission_date
order by index_code, weight desc);

/* 创建视图 */
/* 拼接中证的最新前10权重股+每月聚宽中的10位之后的权重股 */
create view mix_top10_with_bottom as
(select index_code, index_name, global_stock_code,stock_code, stock_name, weight, source, submission_date from (
/* 拼接中证的最新前十权重股 */
(select index_code, index_name, global_stock_code,stock_code, stock_name, weight, source, submission_date
from index_constituent_stocks_weight
where source = '中证'
...
(select a.index_code, index_name, global_stock_code, stock_code, stock_name, weight, source, a.submission_date from jq a
where
( select count(1) from jq b
				where a.index_code = b.index_code
				and a.weight<b.weight
				order by b.weight desc) >= 10
group by a.index_code, index_name, global_stock_code, stock_code, stock_name, weight, source, a.submission_date
...
(select mt10wb.index_code, index_name, mt10wb.global_stock_code, mt10wb.stock_code, stock_name, weight, source, mt10wb.submission_date
...
group by mt10wb.index_code, index_name, mt10wb.global_stock_code, mt10wb.stock_code, stock_name, weight, source, mt10wb.submission_date
