...
class CalculateIndexHistoricalEstimations:
    ...
    def cal_one_index_estimation_in_a_special_day(self, index_constitute_stocks, index_code, index_name, day):
        ...
        print(threading.current_thread().name, index_code, index_name, day)
        ...
        for index_code in target_indexes:
            ...
            with threading_pool:
                ...
                for day in days:
                    ...
                    # 启动线程
                    t = threading.Thread(target=self.cal_one_index_estimation_in_a_special_day,
    ...
    def daily_check_and_cal_all_index_estimation_no_matter_updated_or_not_in_multi_process(self):
        # 每日的例行检查,并计算指数估值, 多进程，每个进程下多线程进行计算
        # 如果指数构成有变化，则重新计算，基于新构成，历史上的每一天的基金估值；
        # 如果指数构成无变化，则基于现有构成情况，计算今天的基金估值情况；
